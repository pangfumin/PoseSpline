...
#include "QuaternionSpline/NumericIntegrator.hpp"
#include <Eigen/Core>
#include <cmath>
#include <iostream>
...
void checkIntegral(TFunctor & f){
    for(int i = 0; i < numberOfBounds; i ++){
        for(int j = 0; j < numberOfBounds; j ++){
            ...
            //CHECK_EQ((val - analyticalVal).norm()<0.01,true);
            std::cout<<"val: "<<std::endl<< val<< std::endl<<"analyticalVal: "<<std::endl<< analyticalVal<<std::endl;
        }
    }
}
...
int main(){
   ...
   //compareWithAnalyticalIntegration)
   ...
   //eigenVectorValueIntegration)
    ...
    double val = integrateFunction(0., 1., Integrand::integrand, 100, 0.);
