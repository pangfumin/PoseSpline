...
TEST(Pose, operate){
    ...
    for (size_t i = 0; i < 100; ++i) {
        ...
        // Test composition
        EXPECT_TRUE(((T_AB * T_BC).Transformation() - T_AB.Transformation() * T_BC.Transformation()).norm() < 1e-8);

        // Test construction
        Pose<double> T_AB_alternative(T_AB.Transformation());
        EXPECT_TRUE((T_AB.Transformation() - T_AB_alternative.Transformation()).norm() < 1e-8);
        Pose<double> T_AB_alternative2(T_AB.r(), T_AB.q());
        EXPECT_TRUE((T_AB.Transformation() - T_AB_alternative2.Transformation()).norm() < 1e-8);

        // Test =
        Pose<double> T_AB_alternative3;
        T_AB_alternative3 = T_AB;
        EXPECT_TRUE((T_AB.Transformation() - T_AB_alternative3.Transformation()).norm() < 1e-8);

        // Test setters
        Pose<double> T_AB_alternative4;
        T_AB_alternative4.set(T_AB.r(), T_AB.q());
        EXPECT_TRUE((T_AB.Transformation() - T_AB_alternative4.Transformation()).norm() < 1e-8);
        Pose<double> T_AB_alternative5;
        T_AB_alternative5.set(T_AB.Transformation());
        EXPECT_TRUE((T_AB.Transformation() - T_AB_alternative5.Transformation()).norm() < 1e-8);
        ...
        for (size_t i = 0; i < 6; ++i) {
            ...
            EXPECT_TRUE((T_AB_p.parameters() - T_AB_p_param).norm() < 1e-8);
            EXPECT_TRUE((T_AB_m.parameters() - T_AB_m_param).norm() < 1e-8);
        ...
        EXPECT_TRUE((local_plus_jacobian - jacobian).squaredNorm() < 1e-8);
        EXPECT_TRUE((local_lift_jacobian*local_plus_jacobian - Eigen::Matrix<double,6,6>::Identity()).squaredNorm() < 1e-8);
