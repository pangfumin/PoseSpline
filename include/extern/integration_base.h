...
class IntegrationBase
{
  ...
  public:
    ...
    /*
     * For midPointIntegration, we can refer to
     * Quaternion kinematics for the error-state
     */
                            ...
                            Eigen::Vector3d &result_linearized_ba, Eigen::Vector3d &result_linearized_bg, bool update_jacobian)
    {
        ...
        if(update_jacobian)
        {
            ...
            // rotation error propagation.Quaternion kinematics for the error-state
            // The rotation convention in this paper,  R = R_bar(I + cross(theta))
            // the theta_{k+1} = (I - cross(omega)*dt)theta_k
            ...
            // this derivation of the transition matrix \Phi is solely based on the equation that $\Phi = exp(F\delta t)\approx I+ F\delta t+ 0.5*(F\delta t)^2$
            // the IMU preintegration dynamic equation is in eq. 9 "Technical Report VINS-Mono: A robust and versatile monocular visual-inertial state estimator"
            //TODO: use the integral method to derive the system transition matrix as done in Huai Dissertation

            MatrixXd Fprime = MatrixXd::Zero(15, 15);
            Fprime.block<3, 3>(0, 6) = MatrixXd::Identity(3,3); //p, v
            Fprime.block<3, 3>(3, 3) = - R_w_x;// theta,theta
            Fprime.block<3, 3>(3, 12) = - MatrixXd::Identity(3,3); // theta, b_g
            Fprime.block<3, 3>(6, 3) = -0.5 * delta_q.toRotationMatrix() * R_a_0_x  +
                    -0.5 * result_delta_q.toRotationMatrix() * R_a_1_x; // v, theta

            Fprime.block<3, 3>(6, 9) = -0.5 * (delta_q.toRotationMatrix() + result_delta_q.toRotationMatrix()); // v, b_a

            MatrixXd FprimeSquared = MatrixXd::Zero(15, 15);
            FprimeSquared.block<3,3>(0,3) = Fprime.block<3, 3>(6, 3);
            FprimeSquared.block<3,3>(0,9) = Fprime.block<3, 3>(6, 9);
            FprimeSquared.block<3,3>(3,3) = R_w_x*R_w_x;
            FprimeSquared.block<3,3>(3,12) = R_w_x;
            FprimeSquared.block<3,3>(6,3) = -Fprime.block<3, 3>(6, 3)*R_w_x;
            FprimeSquared.block<3,3>(6,9) = Fprime.block<3, 3>(6, 9)*Fprime.block<3, 3>(6, 9);
            FprimeSquared.block<3,3>(6,12) = -Fprime.block<3, 3>(6, 3);

            F =  MatrixXd::Identity(15, 15) + Fprime*_dt + 0.5*FprimeSquared*_dt*_dt;

            F.block<3, 3>(0, 3) = -0.25 * delta_q.toRotationMatrix() * R_a_0_x * _dt * _dt +
                    -0.25 * result_delta_q.toRotationMatrix() * R_a_1_x * _dt * _dt +
                    1/12*std::pow(_dt, 3)*(delta_q.toRotationMatrix() * R_a_0_x +
                     result_delta_q.toRotationMatrix() * R_a_1_x)*R_w_x; //also include the third order term

            //                V.block<3, 3>(0, 0) =  0.25 * delta_q.toRotationMatrix() * _dt * _dt;
            //                V.block<3, 3>(0, 3) =  0.25 * -result_delta_q.toRotationMatrix() * R_a_1_x  * _dt * _dt * 0.5 * _dt;
            //                V.block<3, 3>(0, 6) =  0.25 * result_delta_q.toRotationMatrix() * _dt * _dt;
            //                V.block<3, 3>(0, 9) =  V.block<3, 3>(0, 3);
            V.block<3, 3>(3, 3) =  -MatrixXd::Identity(3,3);
            //                V.block<3, 3>(3, 9) =  0.5 * MatrixXd::Identity(3,3) * _dt;
            V.block<3, 3>(6, 0) =  -(0.5 * delta_q.toRotationMatrix() + 0.5 * result_delta_q.toRotationMatrix());
            //                V.block<3, 3>(6, 3) =  -result_delta_q.toRotationMatrix() * R_a_1_x  * _dt * 0.5 * _dt;
            //                V.block<3, 3>(6, 6) =  0.5 * result_delta_q.toRotationMatrix() * _dt;
            //                V.block<3, 3>(6, 9) =  V.block<3, 3>(6, 3);
            V.block<3, 3>(9, 12) = MatrixXd::Identity(3,3);
            V.block<3, 3>(12, 15) = MatrixXd::Identity(3,3);
            jacobian = F * jacobian;
            covariance = F * covariance * F.transpose() + V *_dt* noise * _dt * V.transpose();
#endif
        }
                                          ...
                                          const Eigen::Vector3d &Pj, const Eigen::Quaterniond &Qj, const Eigen::Vector3d &Vj, const Eigen::Vector3d &Baj, const Eigen::Vector3d &Bgj)
    {
        ...
        // When the ba and bg esitimate change slightly,
        // using this linear first-order approximation to correct pre-intergation motion
        // to prevent re-integration.

        Eigen::Vector3d dba = Bai - linearized_ba;  // ba change
        Eigen::Vector3d dbg = Bgi - linearized_bg;  // bg change
    ...
    double sum_dt; // accumulation of dt
