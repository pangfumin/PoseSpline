...
namespace numeric_integrator {
    ...
    public:
        AbstractIntegrator(IntegrationScalar a, IntegrationScalar b, int nIntegrationPoints) :
                a(a),
                b(b),
                maxIndex(nIntegrationPoints - 1) {}
    ...
    /*
     * Curiously Recurring Template Pattern Idioms
     */
    ...
    public:
        template <typename ValueFactor, typename IntegrationScalar>
        Integrator<ValueFactor, IntegrationScalar> getIntegrator(IntegrationScalar a,
                                                                 IntegrationScalar b,
                                                                 int nIntegrationPoints) const
        {
    ...
    namespace internal {
        ...
        public:
            FixStepSizeIntegrator(IntegrationScalar a, IntegrationScalar b, int nIntegrationPoints):
                    AbstractIntegrator<ValueFactor, IntegrationScalar>(a, b, nIntegrationPoints),
                    stepSize((b-a) / this->maxIndex),
                    iIntegrationPoint(0) {}
            inline bool isAtEnd() const { return iIntegrationPoint > this->maxIndex; }
            inline void next() { ++iIntegrationPoint;}
            inline IntegrationScalar getIntegrationScalar() const {
                return (iIntegrationPoint == this->maxIndex) ? this->b : this->a + stepSize * iIntegrationPoint;
            }
            inline bool atBounds() const {
                return this->iIntegrationPoint == 0 || this->iIntegrationPoint == this->maxIndex;
            }
        protected:
        ...
        public:
            SimpsonRuleIntegrator(IntegrationScalar a, IntegrationScalar b, int nIntegrationPoints):
                    FixStepSizeIntegrator<ValueFactor, IntegrationScalar>(a,
                                                                          b,
                                                                          (nIntegrationPoints + 1 - nIntegrationPoints % 2)) {}
            inline ValueFactor getValueFactor() const {
                return this->atBounds() ? 0.5 : (this->iIntegrationPoint % 2 == 1 ? 2 : 1);
            }
        ...
        public:
            TrapezoidalRuleIntegrator(IntegrationScalar a, IntegrationScalar b, int nIntegrationPoints):
                    FixStepSizeIntegrator<ValueFactor, IntegrationScalar>(a, b, nIntegrationPoints) {}
...
//	};
    ...
    inline TValue integrateFunctor(TArgScalar a,
                                   TArgScalar b,
                                   const TFunctor & f,
                                   int numberOfPoints,
        ...
        CHECK_EQ(numberOfPoints>2 && !integrator.isAtEnd(),true)<<"too few integration points given : " << numberOfPoints;
...
//		}
    ...
    inline TValue integrateFunction(TArgScalar a,
                                    TArgScalar b,
                                    TValue (& integrand)(const TArgScalar & t),
                                    int numberOfPoints,
                                    TValue zero = TValue(0)){
