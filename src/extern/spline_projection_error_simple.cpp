...
SplineProjectSimpleError::SplineProjectSimpleError(const SplineProjectSimpleFunctor& functor):
functor_(functor){}
...
bool SplineProjectSimpleError::EvaluateWithMinimalJacobians(double const *const *parameters,
    ...
    if (!jacobians) {
        return ceres::internal::VariadicEvaluate<
                SplineProjectSimpleFunctor, double, 7, 7, 7, 7, 3,0,0,0,0,0>
        ::Call(functor_, parameters, residuals);
    }


    bool success =  ceres::internal::AutoDiff<SplineProjectSimpleFunctor, double,
            7,7,7,7,3>::Differentiate(
            functor_,
            parameters,
            2,
            residuals,
    ...
    if (success && jacobiansMinimal!= NULL) {
        ...
        if( jacobiansMinimal[4] != NULL){
            Eigen::Map<Eigen::Matrix<double,2,3>> J4_minimal_map(jacobiansMinimal[4]);
            Eigen::Map<Eigen::Matrix<double,2,3>> J4_map(jacobians[4]);
